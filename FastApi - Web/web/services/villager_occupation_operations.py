...
class VillagerOccupationService:
    def __init__(self, session: Session = Depends(get_session)):
        self.session = session

    def _get_human(self, human_id: int):
        this_id = (self.session.query(VillagerOccupation).filter_by(villager_id=human_id).first())
        if not this_id:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
        return this_id

    def create(self, table_data: VillagerOccupationModel):
        new_table_data = VillagerOccupation(**table_data.dict())  # распаковываем словарь в аргументы конструктора
    ...
    def read(self, table_id):
        ...
        if table_id == "Villager Occupation":
    ...
    def update(self, human_id: int, table_data: VillagerOccupationModel):
